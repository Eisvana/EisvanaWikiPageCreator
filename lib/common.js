...
const numberFieldKeys = '-.0123456789'
...
function isNumberKey(input) {
	...
	const valuesWithoutDot = newValue.split('.');
	if (valuesWithoutDot.length > 2) {
		newValue = newValue.substring(0, newValue.length - 1);
	}
	else if (valuesWithoutDot.length == 2) {
	...
	input.value = newValue
		.split('')
		.filter(char => numberFieldKeys.includes(char))
		.join('')
}
