...
function toggleSection(sectionName, button, attributeName = 'section') {
	...
	const id = button.dataset.buttonId;
	const state = button.dataset[`display${id}`];
	...
	for (const element of elements) {
		...
		for (const key in object) {
			if (!key.includes('display') || key == `display${id}`) continue;
			if (object[key] == 'none') {
				hide = true;
				break;
			}
		...
		if (!element.dataset[attributeName].split(' ').includes(sectionName)) continue;
		if (state) {
			element.style.display = '';
			element.dataset[`display${id}`] = '';
		} else {
			element.style.display = 'none';
			element.dataset[`display${id}`] = `none`;
		}
	}
	button.innerText = state ? 'Hide' : 'Show';
	button.dataset[`display${id}`] = state ? '' : 'none';
}
...
function enableTextMarking() {
	document.body.dataset.mark = !checkDataIntegrity();
}

globalElements.output.output.onclick = () => {
	const error = checkDataIntegrity();
	if (error) {
...
function getChildIndex(array, data) {
	...
	for (const element of array) {
		const idNumber = fetchFromObject(element, data).match(regex).join('');	// get all numbers of the string into an array, then join that array
