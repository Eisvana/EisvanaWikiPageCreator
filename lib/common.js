...
function validateCoords() {
	...
	const axesNumbers = axesValues.map(x => (Math.abs(parseInt(x)).toString()))		// removes +/- prefixes: ['xx', 'xx', 'xx', 'xx']
	if (!(axesNumbers[1].length == 2 && axesNumbers[3].length == 2 && axesNumbers[0].length >= 2 && axesNumbers[0].length <= 3 && axesNumbers[2].length >= 2 && axesNumbers[2].length <= 3)) {
