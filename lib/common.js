...
function wikiLink(codeId, inputId) {
	...
	let link = 'https://nomanssky.fandom.com/wiki/' + name + '?action=edit';
	if (!name == '') {
		a.href = link;
...
function civRegDropdown(civ, region) {
	let check = document.getElementById(civ).value;
	document.getElementById(region).innerHTML = "";
	switch (check) {
		case "GHub":
			setDropDownOptions(GHubRegions, region);
			break;

		case "CalHub":
			setDropDownOptions(CalHubRegions, region);
			break;

		case "EisHub":
			setDropDownOptions(EisHubRegions, region);
			break;
	}
}
...
function downloadFile(buttonId, codeId, inputId) {
	...
	let downloadFileContent = document.getElementById(codeId).innerText;
	let downloadFileContent = downloadFileContent.replaceAll('\n\n\n', '\n\n');
...
function getHubNumber(region_input, hubCodeId) {
	let region = document.getElementById(region_input).value;
	let check = document.getElementById('civ').value;
	let index = ''
	switch (check) {
		case "GHub":
			index = GHubRegions.indexOf(region);
			if (index > 10) {
				index = 'Huburb'
			} else {
				index++
		...
		case "CalHub":
			index = CalHubRegions.indexOf(region);
			index++
			break;

		case "EisHub":
			index = EisHubRegions.indexOf(region);
			index++
			break;
	}
	setOutput(hubCodeId, index)
}
...
function hideDiscoverer(discovered, discoveredlink) {
	...
	if(!discoveredlinkInput.value == '') {
		...
		discoveredInput.parentElement.parentElement.style.display = 'table-row'
	};
	if(!discoveredInput.value == '') {
		discoveredlinkInput.parentElement.parentElement.style.display = 'None'
	} else {
		discoveredlinkInput.parentElement.parentElement.style.display = 'table-row'
	};
}

// applies a/an based on given input string
function enPrefix(inputId, codeId) {
	let type = document.getElementById(inputId).value.substring(0, 1);
