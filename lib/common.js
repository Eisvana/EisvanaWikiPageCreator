...
function startUp() {
	...
	globalElements.output.output.ontouchstart = () => preventCopy();	// this must be first
	...
	if (globalElements.output.albumText) globalElements.output.albumText.ontouchend = (e) => getSelectedText(e.target);
	if (globalElements.output.albumText) globalElements.output.albumText.onmouseup = (e) => getSelectedText(e.target);
...
function showAll() {
	...
	hideDiscoverer();
	try { startupFunctions() } catch (error) { console.warn(error)/*do nothing*/ };
}
...
function checkDataIntegrity(simple = false) {		// returns false if nothing is wrong
	...
	if (name && (glyphs || pageType == 'Business') && region && ((currentText == savedText && dataIntegrityObj.copy === true) || simple)) {
		dataIntegrityObj.copy = false;
		return false;
	} else if (!name) {
		return 'Missing name!';
	} else if ((!glyphs || !region) && pageType != 'Business') {
		return 'Wrong glyphs!';
	} else {
...
function getSelectedText(section) {
	// this is some stupid BS: Chrome selects the button text, despite it having user-select:none.
	...
	const sectionText = section.closest('.wikiText').innerText.replace(newlineRegex, '').trim();
	const selected = (() => {
