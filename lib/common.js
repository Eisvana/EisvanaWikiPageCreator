...
function setOutput(codeId, textToPut) {
	if (!document.getElementById(codeId)) return;
	const text = sanitiseString(textToPut);
	document.getElementById(codeId).innerHTML = text;
}

// places something at multiple spots in the code
function setOutputMultiple(codeName, textToPut) {
...
// adds bullet point if more than one sentence; also has whitespace option
function addInfoBullet(name, linebreak) {
	...
	for (const line of lines) {
		if (lines.length > 1) {
			line.innerHTML = '*';
			if (linebreak) line.innerHTML = '<br>';
		} else {
			line.innerHTML = '';
		}
	}
