...
function assignElementFunctions(object) {
	for (const elementId in object) {
		...
		assignFunction(element, functionName, object[elementId][1]);
	}
}
...
function assignFunction(element, functionName, listener = null) {
...
function validateCoords() {
	...
	const axesComponents = axes.split(', ');
	...
	const axesValues = new Array;
	for (const axesComponent of axesComponents) {
		const axesNumbers = axesComponent.split('.');
		for (const axesNumber of axesNumbers) {
			axesValues.push(axesNumber);
		}
	}
	if (axesValues.map(Number).join().includes('NaN')) {
	...
	if (!(axesValues[1].length == 2 && axesValues[3].length == 2 && axesValues[0].length >= 1 && axesValues[0].length <= 3 && axesValues[2].length >= 1 && axesValues[2].length <= 3)) {
