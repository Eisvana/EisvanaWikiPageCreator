...
function researchTeam(researchTeam_codeId, civ_codeId) {
	...
	if (researchteam_input == '' || researchteam_input.length > 4) {
...
function formatName(documenter) {
	let documented = documenter;
	if (documented == '' || documented.includes('[') || documented.includes('{')) {
...
function displayResearch(researchteam) {
	let chapter = researchteam;
	if (chapter == '') {
		chapter = '';
	} else if (chapter.length == 4) {
...
function docBy(codeId, inputId, discovered_inputId, discoveredlink_inputId, researchTeam_inputId) {
	...
	chapter = displayResearch(chapter);
