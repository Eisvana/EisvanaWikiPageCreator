...
function sanitizeString(input) {
	...
	while (text.includes('[http')) { 
		...
		text = text.replace('[http', linkStartReplacement)
	}

	for (const markup of doubleWikiMarkup) {
		text = text.replaceAll(`${markup}${markup}`, tempReplacement)
		text = text.replaceAll(markup, '')
		text = text.replaceAll(tempReplacement, `${markup}${markup}`)
	}
	text = text.replaceAll(linkStartReplacement, '[http')
	text = text.replaceAll(linkEndReplacement, ']')
	return text;
}
	...
	for (let fileIndex = 0; fileIndex < inp.files.length; ++fileIndex) {
		...
		const galleryTemplate = `<div id="${galleryId}" class="gallery-item">
			...
			<div class="controlButtons">
				...
				<button onclick="moveItem('${galleryId}', '${wikiCodeGalleryValueId}', 'up')">
					<svg width="36" height="36"><path d="M2 25h32L18 9 2 25Z"></path></svg>
				</button>
				<button onclick="moveItem('${galleryId}', '${wikiCodeGalleryValueId}', 'down')">
...
function moveItem(elementId, codeId, direction) {
	let galleryItem = document.getElementById(elementId)
	let galleryCodeItem = document.getElementById(codeId)
    const elements = [galleryItem, galleryCodeItem];
	for (const element of elements) {
		const span = element.parentNode;
		const td = span.parentNode;
		
		if (direction == 'up' && span.previousElementSibling) {
			td.insertBefore(span, span.previousElementSibling);
		} else if (direction == 'down' && span.nextElementSibling) {
