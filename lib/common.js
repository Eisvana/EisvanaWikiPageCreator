...
function galleryUpload(uploadId, descId, codeId) {
	...
	for (let fileIndex = 0; fileIndex < inp.files.length; ++fileIndex) {
		...
		const childIndex = (inputDiv.childNodes?.length ?? 0);
...
function researchTeam(researchTeam_codeId, civ_codeId) {
	...
	if (researchteam_input == '') {
		researchteam = document.getElementById(civ_codeId).innerHTML;
	} else if (researchteam_input.includes('Scribe')) {
		researchteam = 'Galactic Hub Eissentam Scribes'
	} else if (researchteam_input.includes('Archivist')) {
		researchteam = 'Galactic Hub Calypso Archivists'	
	}
