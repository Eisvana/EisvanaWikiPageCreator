...
function getHubNumber(region_input, hubCodeId) {
	...
	switch (check) {
		case "GHub":
			var index = GHubRegions.indexOf(region);
			if (index > 10) {
				index = 'Huburb'
			} else {
				index++
		...
		case "CalHub":
			var index = CalHubRegions.indexOf(region);
			index++
			break;

		case "EisHub":
			var index = EisHubRegions.indexOf(region);
			index++
			break;
	}
...
function hideDiscoverer(discovered, discoveredlink) {
	var discoveredInput = document.getElementById(discovered)
	var discoveredlinkInput = document.getElementById(discoveredlink)
...
function enPrefix(inputId, codeId) {
	var type = document.getElementById(inputId).value.substring(0, 1);
