...
const elementFunctions = {
	researchTeam: ['researchTeam()', 'onchange'],
	discoveredInput: ['hideDiscoverer("discoveredInput", "discoveredlinkInput")'],
	discoveredlinkInput: ['hideDiscoverer("discoveredlinkInput", "discoveredInput")'],
}
function assignFunctions(object) {
	for (const elementId in object) {
		...
		const type = object[elementId][1] ?? 'oninput';
...
function displayResearch(researchteam_inputId) {
	...
	const teamIDs = document.getElementById(researchteam_inputId).children;
...
function hideDiscoverer(keepId, removeId) {
	const keepInput = document.getElementById(keepId);
	const removelinkInput = document.getElementById(removeId);
	const removelinkInputCell = removelinkInput.parentElement;
	const removeLabelCell = removelinkInputCell.previousElementSibling;
	...
	removelinkInputCell.style.display = showStatus;
	removelinkInput.value = '';
