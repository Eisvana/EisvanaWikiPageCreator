...
function docBy() {
	...
	const discoverer = pageData.discovered ?? pageData.builder ?? pageData.owner;
	const discoveredlink = pageData.discoveredlink ?? pageData.builderlink ?? pageData.ownerLink;
...
function numberStats(element, decimals = 0) {
	const dest = element.dataset.destNoauto
	const propertyValue = pageData[dest];

	const propertyData = getNumber(propertyValue, decimals);
	if (!propertyData && propertyData != propertyValue) {
