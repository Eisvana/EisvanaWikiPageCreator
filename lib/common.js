...
function showAll() {
	...
	hideDiscoverer();
	try { startupFunctions() } catch (error) { console.log(error)/*do nothing*/ };
}

function wikiCode(element, destination = null) {
	const dest = element.dataset.dest ?? destination;
	const destElements = Array.from(document.getElementsByName(dest));
...
function image(element) {
	...
	if (!filename) return;
	const fileInput = globalElements.input.fileInput;
...
// returns true (bool) if string is a number
function isNumber(string) {
	const numberRegex = new RegExp(/^\d+$/);
	return regexMatch(string, numberRegex);
}