...
// gets a value from the wikicode
function getCodeValue(codeId) {
	return document.getElementById(codeId).innerHTML
}

// places something in the code
function setOutput(codeId, textToPut) {
	const text = sanitiseString(textToPut)
...
function textFunction(codeId, inputId) {
	...
	text = sanitiseString(text).replaceAll('<br>', ' ')
	document.getElementById(codeId).innerHTML = text
}

// get text from textarea fields
function textAreaFunction(codeId, inputId) {
	let text = document.getElementById(inputId).value
	text = sanitiseString(text)
	document.getElementById(codeId).innerText = text
}

// copies text from code to other place in code
function codeFunction(codeId_new, codeId_org) {
	let text = document.getElementById(codeId_org).innerHTML
	text = input = sanitiseString(text)
	document.getElementById(codeId_new).innerHTML = text
}

// distributes text from code to other place in code
function codeFunctionMultiple(codeName_new, codeId_org) {
	const outputs = document.getElementsByName(codeName_new)
	let input = document.getElementById(codeId_org).value
...
// distributes a value to all occurrences of a certain Name
function textFunctionMultiple(codeName, inputId) {
	const outputs = document.getElementsByName(codeName)
	let input = document.getElementById(inputId).value
	input = sanitiseString(input).replaceAll('<br>', ' ')
	for (const output of outputs) {
		output.innerHTML = input
	}
}
...
function floatFunction(codeId, inputId, decimals) {
	const number = document.getElementById(inputId).value;
	if (!isNaN(number) && number != '') {
		let output = parseFloat(number).toFixed(decimals);
		output = new Intl.NumberFormat('en-US').format(output)
...
function sanitiseString(input) {
	...
	while (text.includes('[http')) {
		let preString = "[http";
		let searchString = "]";
		let preIndex = text.indexOf(preString);
		let searchIndex = text.indexOf(searchString, preIndex);
		text = text.replaceAt(searchIndex, linkEndReplacement)
			.replace('[http', linkStartReplacement)
	}
...
function resetAll(galleryId) {
	...
	const child = document.getElementById(galleryId)
...
function copyText(codeId, inputId) {
	...
	const buttonText = document.getElementById(inputId).innerHTML;
	navigator.clipboard.writeText(copyTextContent.replaceAll('\n\n\n', '\n\n'))

	document.getElementById(inputId).innerHTML = 'Copied!'
	setTimeout(() => {
		document.getElementById(inputId).innerHTML = buttonText
	}, 1500)
}
...
function dateConvert(codeId) {
	const text = document.getElementById(codeId).innerHTML
	document.getElementById(codeId).innerHTML = text.replaceAll("-", "/")
}

// creates a wiki link for your page
function wikiLink(codeId, region_codeId, name_inputId, prepend) {

	let name = document.getElementById(name_inputId)?.value;
	name = sanitiseString(name);
	const region = document.getElementById(region_codeId).innerHTML;
	const buttonText = document.getElementById(codeId).innerHTML;
	const a = document.getElementById(codeId);
	const link = 'https://nomanssky.fandom.com/wiki/Special:EditPage/' + (prepend ? prepend + ' ' : '') + name;
...
function setDropDownOptions(options, value, dropdown) {
...
function downloadFile(buttonId, codeId, inputId) {
	...
	const downloadFileContent = document.getElementById(codeId).innerText.replaceAll('\n\n\n', '\n\n');
...
function imgName(inputId, textInputField) {
	let imgName = document.getElementById(inputId).value.split("\\").pop()
	document.getElementById(textInputField).value = imgName
}
...
function moveItem(elementId, codeId, direction) {
...
function researchTeam(researchTeam_codeId, civ_codeId) {
	const researchteam_input = document.getElementById(researchTeam_codeId).innerHTML;
	const civ = document.getElementById(civ_codeId).innerHTML;
	if (researchteam_input.length == 3 || researchteam_input.length == 4 || researchteam_input.split(' ').length == 3) return

	let researchteam;
	if (researchteam_input == '') {
		researchteam = civ;
	} else if (researchteam_input.split(' ').length > 1) {
		researchteam = civ + ' ' + researchteam_input.split(' ')[1]
	}
	document.getElementById(researchTeam_codeId).innerHTML = researchteam;
}
...
function docBy(codeId, inputId, discovered_inputId, discoveredlink_inputId, researchteam_inputId) {
	...
	if (documenter != '' && documenter != discoverer && documenter != discoveredlink) {
		...
		setOutput(codeId, `Documented by ${chapter} ${documenter}.<br><br>`)
	} else {
		document.getElementById(codeId).style.display = 'none';
	}
}

// only accepts numbers and decimal points in input fields
function isNumberKey(evt) {
	let charCode = (evt.which) ? evt.which : evt.code;
...
// finds missing whitespace
function whitespaceDetector(parentElement_Id) {
	let parent = document.getElementById(parentElement_Id)
	let children = document.querySelectorAll(`#${parentElement_Id} > div`)
	let brElement = document.createElement('br');
	brElement.id = 'brElement'
	...
	if (brArr.length > 0) {
		if (parent.contains(document.getElementById('brElement'))) {
			document.getElementById('brElement').remove()
		}
	} else {
		if (!parent.contains(document.getElementById('brElement'))) {
...
// replaces a specific index in a string
String.prototype.replaceAt = function (index, replacement) {
	return this.substring(0, index) + replacement + this.substring(index);
}

// puts checkbox value into code if checked
function checkBoxFunction(codeId, checkboxId) {
	const checkbox = document.getElementById(checkboxId)
	if (checkbox.checked) {
		setOutput(codeId, checkbox.value)
	} else {
		setOutput(codeId, '')
	}
}
...
function checkBoxFunctionMultiple(checkboxName) {
...
// validates the discord name formatting (discriminator included?)
function discordValidation(input, error) {
	if (parseInt(input.value.substring(input.value.length - 4)) == input.value.substring(input.value.length - 4) && input.value.substring(input.value.length - 5, input.value.length - 4) == '#') {
...
function planetMoon(planet_inputId, moon_inputId, codeId, noLink) {
	let text;
	const moon = sanitiseString(document.getElementById(moon_inputId).value);
	const planet = sanitiseString(document.getElementById(planet_inputId).value);
	if (!moon == '') {
		text = `[[moon]] [[${moon}]]`;
	} else {
		text = `[[planet]] [[${planet}]]`;
...
// hides specific input(s) if trigger input value is not empty or is specified value that is not an ID in the HTML doc
function hideInput(trigger_inputId, ...targetInput) {
	const trigger = document.getElementById(trigger_inputId).value;

	const targets = [];
	const matches = [];

	for (const target of targetInput) {
	...
	if (matches.includes(trigger)) {
		...
		for (const target of targets) {
			document.getElementById(target).parentElement.parentElement.style.display = '';
		}
	}
}
