...
function addInfoBullet(name, linebreak) {
	...
	for (const element of elements) {
		...
		element.innerHTML = '';
		if (linebreak && elements[0].parentElement.childElementCount > 2) {
			elements[0].parentElement.lastElementChild.remove();
			elements[0].parentElement.lastElementChild.remove();
		}
	}
	for (const line of lines) {
		if (lines.length > 1) {
			if (linebreak) {
				elements[0].parentElement.insertAdjacentHTML('beforeend', '<br><br>');
				return
			}
			line.innerHTML = '*';
		} else {
