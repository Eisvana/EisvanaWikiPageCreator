...
function genusProduces(genus_inputId, produces_inputId) {
	const genus = document.getElementById(genus_inputId).value
	const ecosystems = [GroundGenera, FlyingGenera, UnderwaterGenera];
	for (const ecosystem of ecosystems) {
		if (Object.keys(ecosystem).findIndex(element => element.includes(genus)) != -1) {
			const food = Object.values(ecosystem)[Object.keys(ecosystem).findIndex(element => element.includes(genus))]
			document.getElementById(produces_inputId).innerHTML = ''
			setDropDownOptions(food, produces_inputId)
...
function specialNotesTextFunc(notes_input, specialNotes_input, addObservation) {
	const notes = document.getElementById(notes_input).value
	const specialNotes = document.getElementById(specialNotes_input).value
