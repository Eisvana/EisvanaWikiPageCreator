...
function merchantUpgrades(MTName, SSName, ECName, ESName, codeId) {
	const checkboxes = {
		MTCheckboxes: checkBoxFunction(MTName),
		SSCheckboxes: checkBoxFunction(SSName),
		ECCheckboxes: checkBoxFunction(ECName),
		ESCheckboxes: checkBoxFunction(ESName)
	}
	const Upgrades = [];

	for (checkbox of Object.keys(checkboxes)) {
		for (let i = 0; i < checkboxes[checkbox].length; i++) {
			const Upgrade = checkboxes[checkbox][i]
			const type = checkbox.substring(0, 2)
			const output = `| ${type}${i + 1}=${Upgrade}<br>`
			Upgrades.push(output)
		}

	}
	setOutput(codeId, Upgrades.join(''))
	console.log(document.getElementById(codeId).innerText)
}