...
function loc(system_inputId, type_inputId, civ_inputId, region_codeId, codeId) {
	...
	function regNr(region_codeId) {
		...
		if (getHubNumber(region_codeId) == 'Huburb') {
			...
			hubNr = ` ${getHubNumber(region_codeId)}`
		}
		return hubNr
	}
	
	function shipType(type_inputId) {
		let type = document.getElementById(type_inputId).value.toLowerCase();
		if (!type == 'freighter') {
			type = 'starship'
		}
		return type
	...
	function HubGal(civ) {
		let text = '';
		civilization = document.getElementById(civ).value
		switch (civilization) {
			case "GHub":
				text = '[[Galactic Hub]]'
				break;
			...
			case "EisHub":
				text = 'the [[Galactic Hub Calypso]], in the [[Eissentam]] [[galaxy]]'
				break;
		}
		return text
	}
...
//Freighter
`This ${shipType(type_inputId)} was discovered in the [[${systemName}]] [[star system]] in the [[${regionName}]] [[region]]${hubNr} of the ${HubGal(civ)}.`
//Capital
