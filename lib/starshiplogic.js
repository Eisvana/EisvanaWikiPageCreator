...
for (var i of GHubHuburbRegions) {
    GHubRegions.push(i);
};
...
function calcS(econ_input, codeId, type_input) {
	...
	var type = document.getElementById(type_input).value
		if (type == 'Exotic' || type == 'Living Ship') {
			var normalChance = 'always spawns'
		} else {
			switch (econ[0]) {
				case "★★★":
					var chance = '2%';
			...
			var normalChance = `has a ${chance} chance to spawn`
		}
	document.getElementById(codeId).innerHTML = 'This starship ' + normalChance + ' in S-class.'
}
