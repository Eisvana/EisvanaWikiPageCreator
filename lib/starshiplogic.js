...
function costSlotCalc(type_inputId, inventory_codeId, cost_codeId, slots_codeId, techslots_codeId, cargoslots_codeId) {
	...
	switch (inventory) {
		case "Small":
			...
			setOutput(techslots_codeId, window[type].TechSmall)
			setOutput(cargoslots_codeId, window[type].CargoSmall)
			break;
			
		case "Medium":
			setOutput(cost_codeId, window[type].CostMedium)
			setOutput(slots_codeId, window[type].SlotsMedium)
			setOutput(techslots_codeId, window[type].TechMedium)
			setOutput(cargoslots_codeId, window[type].CargoMedium)
			break;
			
		case "Large":
			setOutput(cost_codeId, window[type].CostLarge)
			setOutput(slots_codeId, window[type].SlotsLarge)
			setOutput(techslots_codeId, window[type].TechLarge)
			setOutput(cargoslots_codeId, window[type].CargoLarge)
			break;
	}
}
/*
