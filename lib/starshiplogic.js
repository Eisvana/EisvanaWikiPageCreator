...
function calcS(econ_input, codeId, type_input) {
	var econ = document.getElementById(econ_input).value.split(' ');
	var type = document.getElementById(type_input).value
	if (type == 'Exotic' || type == 'Living Ship') {
		var chanceSentence = 'always spawns'
	} else {
		switch (econ[0]) {
			case "★★★":
				var chance = '2%';
		...
		var chanceSentence = 'has a ' + chance + ' chance to spawn'
	}
...
function showHideStarshipSelects(type_inputId, subtype_inputId, exotic_inputId, pilot_inputId, maneuver_inputId, damage_inputId, shield_inputId, warp_inputId, inventory_inputId, planet_inputId, moon_inputId, axes_inputId, economy_inputId) {
	...
	let subtypetrDisplay = no;
	let exotictrDisplay = no;
	let pilottrDisplay = no;
	let inventorytrDisplay = no;
	let maneuvertrDisplay = no;
	let damagetrDisplay = no;
	let shieldtrDisplay = no;
	let warptrDisplay = no;
	let economytrDisplay = no;
	let planettrDisplay = no;
	let moontrDisplay = no;
	let axestrDisplay = no;

	const type = document.getElementById(type_inputId).value;
	switch (type) {
		case "Freighter":
			subtypetrDisplay = yes;
			pilottrDisplay = yes;
			economytrDisplay = yes;
		...
		case "Exotic":
			...
			exoticValue = 'Royal';
			inventoryValue = 'Small';
			break;

		default:
			subtypetrDisplay = yes;
			inventorytrDisplay = yes;
			economytrDisplay = yes;
	}

	setShipInputValueAndDisplay(subtype_inputId, subtypeValue, subtypetrDisplay);
	setShipInputValueAndDisplay(exotic_inputId, exoticValue, exotictrDisplay);
	setShipInputValueAndDisplay(pilot_inputId, pilotValue, pilottrDisplay);
	setShipInputValueAndDisplay(inventory_inputId, inventoryValue, inventorytrDisplay);
	setShipInputValueAndDisplay(maneuver_inputId, maneuverValue, maneuvertrDisplay);
	setShipInputValueAndDisplay(damage_inputId, damageValue, damagetrDisplay);
	setShipInputValueAndDisplay(shield_inputId, shieldValue, shieldtrDisplay);
	setShipInputValueAndDisplay(warp_inputId, warpValue, warptrDisplay);
	setShipInputValueAndDisplay(economy_inputId, economyValue, economytrDisplay);
	setShipInputValueAndDisplay(planet_inputId, planetValue, planettrDisplay);
	setShipInputValueAndDisplay(moon_inputId, moonValue, moontrDisplay);
	setShipInputValueAndDisplay(axes_inputId, axesValue, axestrDisplay);
}

function setShipInputValueAndDisplay(inputId, value, displayValue) {
	const inputNode = document.getElementById(inputId);
	inputNode.value = value;
	const tableRow = inputNode.parentElement.parentElement;
	tableRow.style.display = displayValue;
}
