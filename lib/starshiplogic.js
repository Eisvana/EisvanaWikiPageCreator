...
function loc(system_inputId, type_inputId, civ_inputId, moon_inputId, planet_inputId, axes_inputId, region_codeId, inventory_codeId, codeId) {
	...
	function locText(inventory_codeId, moon_inputId, planet_inputId, axes_inputId) {
		...
		switch (type) {
			...
			case 'Living Ship':
				...
				locationText = `on the [[${planetName}]] ${celestialBody} at the coordinates ${axes}`
				break;
	...
	const output = `This ${shipType()} was discovered in the [[${systemName}]] [[star system]] in the [[${regionName}]] [[region]]${regNr(region_codeId)} of the ${HubGal(civ_inputId)}.<br><br>It can be found ${locText(inventory_codeId, moon_inputId, planet_inputId, axes_inputId)}.`
