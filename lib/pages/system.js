...
const systemElementFunctions = {
	navFileInput: ['assignDefaultValue(this, "NmsMisc_NotAvailable.png")'],
...
function merchantUpgrades(group = null) {
	...
	if (group) getCheckedBoxes(group);

	if (checkboxes[0].onchange) return;

	// if the checkboxes have no onchange event (i.e. at page load), assign them one
	const merchants = new Set();
	for (const checkbox of checkboxes) {
		assignFunction(checkbox, 'merchantUpgrades(this.dataset.destCheckboxGroup)');
	...
	function getCheckedBoxes(group) {
		...
		const parm = group.substring(0, 2);
...
function tradeables() {
	...
	<div class="tableCell data" data-tradeable="section${childIndex}">
		<input type="text" id="${price_input}" data-dest-noauto="${price}" oninput="numberStats(this)">
	</div>`;
	...
	for (const input of noautoInputs) {
		assignFunction(input, 'storeData(this)', null, true);
	}
}
