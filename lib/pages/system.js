...
function addResourceInput(element, sectionTarget) {
	...
	const inputHTML = `<div class="tableCell text ${oddEvenClass}" data-section="resource${sectionTarget} planet${sectionTarget}" data-resource="section${childIndex}" data-planet="planet${sectionTarget}">
		...
		<button class="button is-danger is-outlined" type="button" disabled onclick="removeSpecificSection('section${childIndex}', 'resource'); enableAddResourceButton(${sectionTarget})">Remove</button>
	</div>
...
function addResource(element) {
	...
	for (const planetName in resources) {
		...
		for (const key in planet) {
			...
			if (!usedResources.has(resource)) linkedResources[planetName][key] = `[[${resource}]]`;
			usedResources.add(resource);
			console.log(resource)
		}
	}
	for (const key in resources) {
		const output = Object.values(linkedResources[key]).join('<br>');
