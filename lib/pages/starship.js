const starshipElementFunctions = {
	typeInput: ['subtypeDropdown(); showHideStarshipSelects()'],
}
assignFunctions(starshipElementFunctions);
...
function calcS(econ_input, codeId, type_input) {
	...
	document.getElementById(codeId).innerText = chanceSentence
}
...
function showHideStarshipSelects() {
	...
	for (const input of Object.keys(shipData[type].sections)) {
		...
		if (data.length == 2) inputElement.value = data[1];
...
function invDropdown() {
	...
	if (type == 'Hauler') {
		...
		if (shipData.Hauler.subtypes[subtype].length == 1) {
			document.getElementById(inventory_inputId).value = shipData.Hauler.subtypes[subtype][0];
...
function calcInv() {
	...
	const shipData = getShipData();
	let inventory = pageData.inventory
	switch (type) {
		case "Freighter":
