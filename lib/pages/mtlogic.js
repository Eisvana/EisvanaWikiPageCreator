...
function appearance(name_inputId, type_inputId, colour1_inputId, colour2_inputId, appearance_inputId) {
	...
	const type = document.getElementById(type_inputId).value.toLowerCase();
...
function showSizeDropdown(type_inputId, size_inputId) {
	const type = document.getElementById(type_inputId).value;
	const size = document.getElementById(size_inputId).value;
	console.log(type)
	if (type == 'Experimental') {
		document.getElementById(size_inputId).querySelector('option[value="SMG"]').style.display = 'none';
	} else {
		document.getElementById(size_inputId).querySelector('option[value="SMG"]').style.display = '';
	}
	if (type == 'Experimental' && size == 'SMG') document.getElementById(size_inputId).value = 'Pistol';

	if (type != 'Royal' && type != 'Starter Pistol') {
		document.getElementById(size_inputId).parentElement.parentElement.style.display = '';
	} else {
		document.getElementById(size_inputId).parentElement.parentElement.style.display = 'none';
	}
}

// determine MT type for infobox
function MTType(size_inputId, type_inputId, codeId) {
	const type = document.getElementById(type_inputId).value;
	const size = document.getElementById(size_inputId).value;
	let output;
	...
	setOutputMultiple(codeId, output);
}

// determine if MT is SMG for catalog
function catalogMTType(size_inputId, type_inputId, codeId) {
	const type = document.getElementById(type_inputId).value;
	const size = document.getElementById(size_inputId).value;
	let output;
	switch (type) {
