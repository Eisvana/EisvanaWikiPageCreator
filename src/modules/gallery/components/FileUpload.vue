...
<template>
	<label for="galleryUpload" class="drop-container" :class="{ 'drag-active': dragActive }" @dragenter="dragActive = true"
		@dragleave="dragActive = false" @drop.prevent="dropFile" @dragover.prevent>
		<div>
			<span class="drop-title">Drop gallery files here</span>
			<button v-if="tooltip" class="tooltip" name="galleryFileUploadTooltip">
				<data>There is a preferred order of pictures.</data>
				<data>Gallery</data>
				<data v-html="tooltip"></data>
			</button>
		</div>
		<input ref="galleryUpload" :class="{ error: errors.length }" accept="image/*" type="file" id="galleryUpload"
			multiple @change="uploadFile" />
	</label>
	<div v-if="errors.length" class="error-list">
		<div>The following file(s) exceed the 10MB upload limit and couldn't be added. You can compress them with the <a
				href="https://nmscd.com/Image-Compressor/" target="_blank" rel="noopener noreferrer">Image Compressor</a>.
		</div>
