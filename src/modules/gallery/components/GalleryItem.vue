...
function moveItem(fileItem: FileItem, direction: "up" | "down") {
	...
	if (direction === "up" && indexInStore) {
		...
		direction === "down" &&
		indexInStore !== activeStore.value.length - 1
	) {
...
<template>
	<div class="gallery-item">
		<a class="gallery-media" :href="getUrlObject(fileItem.file)" target="_blank" rel="noopener noreferrer">
			<img :src="getUrlObject(fileItem.file)" :alt="fileItem.file.name" />
		</a>
		<div class="gallery-meta">
			<p style="word-break: break-all">
				<span class="has-text-weight-bold">Name: </span>{{ fileItem.file.name }}
			</p>
			<div v-show="galleryDropdownItems.length">
				<select v-model="fileItem.desc" @mousedown="updateGalleryDropdown" @touchstart="updateGalleryDropdown">
					<option v-for="desc in galleryDropdownItems" :value="desc" :key="desc">
						{{ desc }}
					</option>
				</select>
			</div>
			<div>
				<input type="text" placeholder="Description" v-model="fileItem.desc" />
			</div>
		</div>
		<div class="controlButtons">
			<button class="delete-icon is-clickable" title="Remove picture from gallery" @click="removeItem(fileItem)">
				❌
			</button>
			<img v-show="!isLoc" class="handle" src="/assets/icons/arrow.svg" title="Move picture up or down"
				alt="Move image via drag'n'drop" />
			<button v-show="!isLoc" class="button moveButton" title="Move up" data-move="up"
				@click="moveItem(fileItem, 'up')">
				<svg width="36" height="36">
					<path d="M2 25h32L18 9 2 25Z"></path>
				</svg>
			</button>
			<button v-show="!isLoc" class="button moveButton" title="Move down" data-move="down"
