 ...
 */
export function autoInfested(
  element: HTMLInputElement = globalElements.input.descriptionInput as HTMLInputElement
): boolean | void {
  ...
  (globalElements.output.infested as HTMLOutputElement).innerText = isInfested
    ? '([[Biome Subtype - Infested|Infested]]) '
    : '';
...
export function initialiseSectionInputs(sectionSelector: string) {
  const inputs: NodeListOf<HTMLInputElement | HTMLSelectElement> = document.querySelectorAll(
    `${sectionSelector} :is(input, select)`
  );
  for (const input of Array.from(inputs)) {
    ...
    if (input.dataset.default) {
      assignFunction({
        element: input,
        func: function () {
          assignDefaultValue(this as unknown as HTMLInputElement | HTMLSelectElement);
        },
      });
