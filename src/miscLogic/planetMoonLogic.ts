...
export function addResource(element: HTMLButtonElement = (globalElements.input.resourceInputs as HTMLDivElement).querySelector('button') as HTMLButtonElement) {
	...
	if (resourceInputSectionCount + 1 > 6) {	// NoSonar 6 resources is maximum. Maybe 5 is the max, but I'm not 100% sure
...
function enableResourceAdd() {
	const addButton = (globalElements.input.resourceInputs as HTMLDivElement).querySelector('button') as HTMLButtonElement;
	...
	const evtListeners: ElementFunctions = [
		{
			element: 'removeButton',
			handler: 'click',
			func: function () { removeSpecificSection(`section${i}`, 'fauna'); changeTableEntry(this as unknown as HTMLButtonElement) }
		},
		{
			element: 'hideButton',
			handler: 'click',
			func: function () { toggleSection(`fauna${i}`, this as unknown as HTMLButtonElement) }
		},
		{
			element: 'fileInput',
			handler: 'change',
		...
		},
		{
			...
			handler: 'change',
			func: function () { genusDropdown(this as unknown as HTMLSelectElement) }
		},
		{
			element: 'genusInput',
			handler: 'change',
	...
	const evtListeners: ElementFunctions = [
		{
			element: 'removeButton',
			handler: 'click',
			func: function () { removeSpecificSection(`section${i}`, 'flora'); changeTableEntry(this as unknown as HTMLButtonElement) }
		},
		{
			element: 'hideButton',
			handler: 'click',
			func: function () { toggleSection(`flora${i}`, this as unknown as HTMLButtonElement) }
		},
		{
			element: 'fileInput',
			handler: 'input',
	...
	const eventListeners: ElementFunctions = [
		{
			element: 'removeButton',
			handler: 'click',
			func: function () { removeSpecificSection(`section${i}`, 'mineral'); changeTableEntry(this as unknown as HTMLButtonElement) }
		},
		{
			element: 'hideButton',
			handler: 'click',
			func: function () { toggleSection(`mineral${i}`, this as unknown as HTMLButtonElement) }
		},
		{
			element: 'fileInput',
			handler: 'input',
...
function floraMineralResourceLinks(element: HTMLInputElement | undefined = undefined) {
	...
	for (const planetPropName in sortObj(linkedResources)) {
		...
		for (const destId in sortObj(planetProp)) {
			...
			for (const [resourceInput, resource] of Object.entries(resources)) {	// Carbon, Sodium
...
export function addSandwormTemplate() {
	...
	const dest = element.dataset.destNoauto as string;
	const sections: Array<HTMLDivElement | HTMLOutputElement> = Array.from(document.querySelectorAll(`[data-section="${dest}"]`));
...
export function resetExternal() {
	const sections: NodeListOf<HTMLDivElement> = document.querySelectorAll('[data-moon], [data-resource], [data-fauna], [data-flora], [data-mineral]');
