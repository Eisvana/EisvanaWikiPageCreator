...
export function acquirementGallery() {
  ...
  const InputElementIds = ['srImgInput', 'sysImgInput', 'cabInput', 'coordsInput'];
...
export function autoMTType() {
  ...
  if (
    ...
    !(typeElement instanceof HTMLSelectElement) ||
    !(subtypeInput instanceof HTMLSelectElement)
  )
    return;
...
export function hideAddons() {
  ...
  if (['Royal', 'Sentinel', 'Atlantid'].includes(type)) {
    ...
    const checkboxes: NodeListOf<HTMLInputElement> = addonInput
      .closest('.checkboxes')!
...
export function subtypeDropdown() {
  ...
  const subtypes: {
    ...
    Pistol: [
      ...
      ['Standard', 'Starter Pistol'],
    ],
    Rifle: [
      ['Rifle', 'SMG'],
      ['Standard (Large)', 'SMG (Small)'],
    ],
    Experimental: [
      ['Rifle', 'Pistol'],
      ['Rifle (Large)', 'Pistol (Small)'],
    ],
    Alien: [
      ['Rifle', 'SMG', 'Pistol'],
      ['Rifle (Large)', 'SMG (Medium)', 'Pistol (Small)'],
    ],
  };
