  ...
  switch (type) {
    case 'Freighter':
      if ((shipData[type].subtypes as Array<string>).indexOf(subtype) > 5) {
        // NoSonar up to index 4 the freighters are capital. System freighters start at index 5.
        inventory = 'Small';
      } else {
        inventory = 'Large';
      }
...
export function costSlotCalc() {
  ...
  for (const prop of propArray) {
    (globalElements.output[prop] as HTMLOutputElement).innerText = (shipData[type][prop] as ShipProp)[
      inventory
    ] as string;
  }
}
